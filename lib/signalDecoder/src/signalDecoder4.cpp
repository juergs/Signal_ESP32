...
/*
//--- constructor
void SignalDetectorClass::SignalDetectorClass()
{ 
	buffer[0] = 0; 
	reset(); 
	mcMinBitLen = 17; 	
...
void SignalDetectorClass::SignalDetectorClass(first(buffer), last(NULL), sd_message(4) ): 
{ 
	buffer[0] = 0; 
	reset(); 
	mcMinBitLen = 17; 	
...
void SignalDetectorClass::bufferMove(const uint16_t start)
{
	...
	else if (sd_message.moveLeft(start))
	{
		m_truncated = true; 
		
		//messageLen = messageLen - start;
		messageLen = sd_message.valcount;
	...
	if (!checkMBuffer())
	{
		...
		MSG_PRINT(F(" bytecnt=")); MSG_PRINT(sd_message.bytecount);
		MSG_PRINT(F(" valcnt=")); MSG_PRINT(sd_message.valcount);
		MSG_PRINT(F(" bcnt=")); MSG_PRINT(sd_message.bcnt);
...
inline void SignalDetectorClass::doDetect()
{
		...
		if (fidx >= 0) {
			...
			if (patternLen == cMaxNumPattern)
			{
				...
				if (messageLen > 0 && histo[pattern_pos] > 0) {
					...
					{
						if ( sd_message[i-1] == pattern_pos) // Finde den letzten Verweis im Array auf den Index der gleich ueberschrieben wird
						{
	...
	histo[fidx]++;
		...
		DBG_PRINT(", BC:");	DBG_PRINT(sd_message.bytecount); 
		DBG_PRINT(", vcnt:");	DBG_PRINT(sd_message.valcount);
		DBG_PRINTLN(" ");
#endif
	...
	for (uint8_t idx = 0; idx<patternLen-1; idx++)
	{
		...
		for (uint8_t idx2 = idx + 1; idx2<patternLen; idx2++)
		{
			...
			if (inTol(pattern[idx2], pattern[idx], tol))  // Pattern are very equal, so we can combine them
			{
				...
				{
					if ( sd_message[i] == idx2)
					{
						sd_message.changeValue(i, idx);
						change_count++;
					}
				}
	...
	if (mcDetected == true || messageLen >= minMessageLen) {
	  ...
	  if (mcDetected == false) {
		...
		if (state == syncfound && messageLen >= minMessageLen) // Messages mit clock / Sync Verhaeltnis pruefen
		{
			...
			{
				if ( sd_message[mstartNeu + 1] == sync) {
			...
			//if ( sd_message[messageLen-1] == 7 || mstart > 10) {
			//if (mstart > 30) {
...
void SignalDetectorClass::printOut()
{
	...
	DBG_PRINT(F(", valcnt: ")); DBG_PRINT(sd_message.valcount);
...
void SignalDetectorClass::calcHisto(const uint8_t startpos, uint16_t endpos)
{
	...
	{
		sd_message.getByte(bstartpos,&bval);
		histo[bval & B00001111]++;
		bstartpos++;
	}
	for (uint16_t i = bstartpos; i <= bendpos; ++i)
	{
		sd_message.getByte(i,&bval);
		histo[bval >> 4]++;
		histo[bval & B00001111]++;
	}
	if (endpos % 2 == 0)  // gerade
	{
		sd_message.getByte(bendpos, &bval);
		histo[bval & B00001111]--;
	}
...
bool SignalDetectorClass::isMuMessageRepeat()
{
	...
	for (int8_t p = patternLen - 1; p >= 0; --p)
	{
		...
		if (histo[p] == 1)
		{
			...
			{
				if ( sd_message[i] == p) break;	// p ist kein Wiederholungstrenner, da er schon am Anfang vorkommt
				i++;
			}
			if (i >= 10)
			{
