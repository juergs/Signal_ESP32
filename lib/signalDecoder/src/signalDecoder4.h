...
class SignalDetectorClass
{
	...
	public:				
		...
		BitStore<defMaxMsgSize/2> message;       //--- container, using 4 bit for every value stored. 
		...
		int16_t* 	first = NULL;                             // Pointer to first buffer entry
		...
		typedef BitStore<defMaxMsgSize/2> bitstore_t ; 
		SignalDetectorClass(int16_t*,int16_t*, bitstore_t); 
		...
		bitstore_t sd_message; 
		
	private:
		//--- callback provided by the client via connectCallback().
		rssiCallbackInterface *m_cb;

};	//--- of  SignalDetectorClass
...
class ManchesterpatternDecoder
{
	...
	//private:
		...
		BitStore<50> ManchesterBits;       //--- store, using 1 bit for every value stored. It's used for storing the Manchester bit data in a efficent way
		int8_t longlow;
		int8_t longhigh;
		int8_t shorthigh;
		int8_t shortlow;		
