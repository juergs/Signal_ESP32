...
class Callee : public rssiCallbackInterface
{
	public:
    	...
    	uint8_t cbiRssiCallbackFunction()  {  return cc1101::getRSSI(); } 
};
...
{ 
	...
	tools::EEbufferFill();	
		...
		//--- check for reset
		long timeout = 2500; 
		Serial.print("Reset to AP command? (enter 'reset') "); Serial.print("Timeout:"); Serial.println(timeout);
		...
		if ( Serial.available() )
		{
			...
			{
				Serial.println(F("Invalid command. Command 'reset' for factory-reset only!"));
			}
		}
	...
	{  
		...
		if (fifoBytes > 0) 
		{
			...
			else 
			{
				...
				if (marcstate == 17 || ccmode == 3) 
				{	
					//--- RXoverflow oder LaCrosse?
					if (cc1101::flushrx()) 
					{
...
void print_bank_sum()	// bs - Banksummary
{
	...
	for (i = 0; i <= 9; i++) 
	{
		...
		{		// Bank Aktiv?
			  ...
			  if (ccmodeStr[i2] == '0' && j == 0) 
			  { 
				// ccmode = 0 und keine gueltige Kurzbeschreibung
